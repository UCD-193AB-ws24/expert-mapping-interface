    ...
    const associatedGrants = grantIDs
      ...
      .filter((grant) => {
        ...
        if (!grant.locationIDs.includes(locationID)) {
          console.warn(
            `Grant with ID ${grant.grantID}does not have locationID ${locationID} in list of locationIDs.`
          );
          return false;
        }
        return true;
  ...
  return expertIDs.map((expertID) => {
    ...
    const associatedWorks = workIDs
      ...
      .filter((work) => {
        ...
        if (!work.locationIDs.includes(locationID)) {
          console.warn(
            `Work with ID ${work.workID} does not have locationID ${locationID} in its list of locationIDs.`
          );
          return false;
        }
        return true;
    ...
    return {
      ...
      works: associatedWorks.map((work) => ({
        title: work.title || "Untitled Grant",
